...
import { Loader2, File, FileText, Image as ImageIcon, FileSpreadsheet, Download, Trash2, UploadCloud, X } from 'lucide-react';
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
          ...
          >
            ...
            {uploadMutation.isPending ? (
              ...
              <UploadCloud className={`w-8 h-8 mb-2 ${isDark ? 'text-white/60' : 'text-gray-400'}`} />
            )}
          ...
          {/* List */}
          <div>
            <h3 className="text-sm font-bold uppercase tracking-wider mb-4 opacity-70">Arquivos Salvos</h3>
            ...
            {isLoading ? (
              ...
              <div className={`text-center p-8 rounded-xl border border-dashed ${isDark ? 'border-white/10 text-white/40' : 'border-gray-200 text-gray-500'}`}>
                ...
                {documentos?.map((doc) => (
                  <div key={doc.id} className={`flex items-center justify-between p-4 rounded-xl border transition-colors ${isDark ? 'border-white/10 bg-white/5 hover:bg-white/10' : 'border-gray-200 bg-white hover:border-gray-300 shadow-sm'}`}>
                    <div className="flex items-center gap-4 overflow-hidden">
                      <div className={`p-2 rounded-lg ${isDark ? 'bg-black/40' : 'bg-gray-100'}`}>
                        {getFileIcon(doc.file_type, doc.file_name)}
                      </div>
                      <div className="overflow-hidden">
                        <p className="font-medium text-sm truncate" title={doc.file_name}>{doc.file_name}</p>
                        <div className={`flex items-center gap-2 text-xs mt-0.5 ${isDark ? 'text-white/50' : 'text-gray-500'}`}>
