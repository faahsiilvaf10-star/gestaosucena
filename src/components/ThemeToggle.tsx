  ...
  return (
    ...
    >
      <div className={`rounded-full p-1 transition-all ${isDark ? 'bg-[#27272a] shadow-sm' : ''}`}>
        <Moon size={16} className={`transition-colors ${isDark ? 'text-blue-300' : 'text-gray-400'}`} />
      </div>
      <div className={`rounded-full p-1 transition-all ${!isDark ? 'bg-white shadow-sm' : ''}`}>
        <Sun size={16} className={`transition-colors ${!isDark ? 'text-amber-500' : 'text-white/30'}`} />
      </div>
    </div>
  )
}
