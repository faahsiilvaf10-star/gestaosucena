...
BEGIN
  ...
  IF NOT is_today AND v_speaker_phone IS NOT NULL AND v_speaker_phone != '' THEN
    -- Remove caracteres não numéricos para garantir formato válido
    v_speaker_phone := regexp_replace(v_speaker_phone, '\D', '', 'g');
    
    -- Adiciona 55 se não começar com 55 e tiver 10 ou 11 dígitos
    IF (length(v_speaker_phone) = 10 OR length(v_speaker_phone) = 11) AND v_speaker_phone NOT LIKE '55%' THEN
      v_speaker_phone := '55' || v_speaker_phone;
    END IF;
